...
def user_submitted_ticket_counter(request):
    ...
    if request.user.is_authenticated:
        return {
            'user_submitted_bug_count': Ticket.objects.filter(author=request.user, tag='bug').count(),
            'user_submitted_feature_count': Ticket.objects.filter(author=request.user, tag='feature').count(),
        }
    else:
        return {
...
def user_saved_ticket_counter(request):
    ...
    if request.user.is_authenticated:
        return {
            'user_saved_bug_count': SavedTicket.objects.filter(user=request.user, ticket__tag='bug').count(),
            'user_saved_feature_count': SavedTicket.objects.filter(user=request.user, ticket__tag='feature').count(),
        }
    else:
        return {
